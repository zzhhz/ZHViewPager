...
import com.zzh.viewpager.utils.HGridPageHelper;


/**
 * 可以设置每一页有多少item和多少列的ViewPager
 */
public class HGridViewPager extends HViewPager
{
    public HGridViewPager(Context context)
    {
        super(context);
    }

    public HGridViewPager(Context context, AttributeSet attrs)
    {
        super(context, attrs);
    }
    ...
    private final HGridPageHelper mHelper = new HGridPageHelper();
    ...
    private final PagerAdapter mInternalPagerAdapter = new PagerAdapter()
    {
        ...
        {
            final HGridLayout gridLayout = new HGridLayout(getContext());
